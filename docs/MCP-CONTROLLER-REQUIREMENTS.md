...
- **整个主控客户端同时只允许一个有效的 AI 客户端会话**，不允许多个 AI 同时加入，也不按远端会话分别放行不同 AI。
- 同一个 AI 可以绑定多个 RustDesk 会话；各远端会话独立维护加入状态和控制权。
- 单 AI 按完成认证和初始化的逻辑 MCP 会话识别，不以 TCP 连接数或工具调用数识别。同一逻辑会话的并发 HTTP 请求不算多个 AI。
- 已有有效 AI 会话时，拒绝第二个 AI 建立独立逻辑会话，并明确返回已有 AI 占用的原因；即使第二个 AI 使用同一令牌，也不能绕过限制。
- 人类接管某个远端会话不解除 AI 的绑定，也不释放全局 AI 名额。AI 主动解绑最后一个远端会话后，只要逻辑 MCP 会话仍然有效，也保留该名额。
- 逻辑 MCP 会话结束或失效后才释放名额。持有同一逻辑会话凭据的进程视为同一客户端；首版不尝试识别其内部是否运行多个模型或代理。
...
| AI 主动结束 MCP 会话，或被判定失联 | 清除所有绑定的 AI 已加入，全部归还人类控制，释放全局 AI 名额 | 保留 GUI 会话，取消该 AI 的接管请求与待执行写入并释放输入状态 |
| MCP 停止、令牌重置 | 终止原 MCP 会话，按 AI 离开统一清理 | 保留 GUI 会话；旧逻辑会话及迟到请求不得继续操作，令牌重置另使旧令牌失效 |
...
- 逻辑 MCP 会话通过标准 MCP `ping` 验证活性：默认每 15 秒探测一次，每次最多等待 15 秒。探测超时则终止逻辑会话并统一清理；正常运行期间的失联检测目标上限约为 30 秒。
...
负责 MCP 设置、运行状态、连接信息、AI 加入提示、控制模式、只读交互、接管按钮和批准界面。
...
- [ ] 第二个独立 AI 逻辑会话被拒绝，同一 AI 可管理多个远端会话且控制权相互独立。
