...
const url = new URL(process.env.MCP_TEST_URL ?? 'http://127.0.0.1:37173/mcp');
...
assert(started);
execFileSync('xcrun',['simctl','launch',udid,'com.apple.Preferences']);
...
execFileSync('xcrun',['simctl','launch',udid,'com.carriez.flutterHbb.mcp.validation']);
let resumed;
for(let n=0;n<30;n++) {
