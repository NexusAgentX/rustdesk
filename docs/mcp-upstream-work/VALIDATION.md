...
更新：2026-09-18。所有结果对应明确提交、配置和环境。产品基线目前只有工作区清单解析记录；另有 V002 的隔离依赖实验。下表完整构建及运行尚无通过结果，macOS 构建正在由子代理推进。
...
| B03 | macOS Flutter aarch64 | 进行中 | 未执行 | 原版主线隔离快照，Rust 1.81；桥接及光标测试阶段性通过，完整应用尚无通过结论 |
...
### V003 — 原版 macOS 构建阶段性进展（尚未完成）

- 来源：构建子代理阶段性回报；详细命令、环境与产物由其 MACOS-BASELINE-BUILD.md 归档。
- 基线：仓库外独立的 upstream `5278fcab685723099e9950d56cb605807db5aa2a` 源码快照与固定 hbb_common。
- 已报告通过：当前主线桥接生成（FRB 1.80.1、cargo-expand 1.0.95、Rust 1.81、Flutter 3.24.5）；CI 的 `cursor_size_test.dart` 共 16 项测试；Cargo.lock 与主线逐字节一致。
- 仍进行中：按固定 baseline 准备原生依赖与 Rust Release 编译。没有完整应用构建或运行成功结论，也没有 MCP 产品验收结果。
